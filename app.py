...
def get_gemini_answer(prompt: str, image=None) -> tuple:
    """
    ...
    Args:
        prompt: Text prompt
        image: Optional PIL Image to send to Gemini
    ...
    try:
        ...
        if image is not None:
            # Gemini expects images in a specific format
    ...
    if not st.session_state.get("collection_ready"):
        ...
        if user_msg is not None:
            ...
            with st.status("Thinking…", expanded=False) as status:
                ...
                # Get answer and token usage (pass uploaded image to Gemini for direct analysis)
                if configured:
                    answer, token_usage = get_gemini_answer(prompt, image=pil_img)
                else:
            ...
            try:
                ...
                pass  # Silent failure - don't show database errors to user
